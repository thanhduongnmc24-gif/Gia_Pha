...
export default function MediaScreen() {
  ...
  const dynamicStyles = {
    ...
    resultBox: { backgroundColor: colors.card, padding: 12, borderRadius: 10, marginBottom: 15, borderWidth: 1, borderColor: colors.border },
    charCard: { backgroundColor: colors.card, padding: 10, borderRadius: 8, marginBottom: 8, borderLeftWidth: 3, borderLeftColor: colors.primary, borderWidth: 1, borderColor: colors.border },
    musicCard: { backgroundColor: colors.card, borderRadius: 16, overflow: 'hidden' as const, borderWidth: 1, borderColor: colors.border, marginBottom: 20 },
    musicHeader: { backgroundColor: colors.primary, padding: 20, alignItems: 'center' as const },
    musicContent: { padding: 20, alignItems: 'center' as const },
    musicTitle: { fontSize: 22, fontWeight: 'bold' as const, color: colors.primary, marginBottom: 5 },
    chordBadge: { backgroundColor: colors.iconBg, paddingHorizontal: 8, paddingVertical: 4, borderRadius: 4, fontSize: 12, fontWeight: 'bold' as const, color: colors.text, marginRight: 5, borderWidth: 1, borderColor: colors.border },
    studioGrid: { flexDirection: 'row' as const, flexWrap: 'wrap' as const, justifyContent: 'space-between' as const, marginTop: 10 },
    studioBtn: { width: '48%' as DimensionValue, flexDirection: 'row' as const, alignItems: 'center' as const, justifyContent: 'center' as const, padding: 12, marginBottom: 10, borderRadius: 8, borderWidth: 1, borderColor: colors.border, backgroundColor: colors.iconBg },
    studioBtnText: { fontSize: 11, fontWeight: 'bold' as const, color: colors.text, marginLeft: 5 },
  };
      ...
      <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={{ flex: 1 }}>
        <ScrollView contentContainerStyle={{ padding: 20, paddingBottom: 100 }}>
          ...
          <View style={dynamicStyles.header}>
              ...
              <TouchableOpacity onPress={handleClearAll} style={{padding:8, backgroundColor: colors.iconBg, borderRadius:8}}><Ionicons name="trash-bin-outline" size={24} color={colors.error} /></TouchableOpacity>
          </View>
          ...
          {mediaType === 'music' && (
            <View>
                <View style={dynamicStyles.musicCard}>
                    <View style={dynamicStyles.musicHeader}><Ionicons name="musical-notes" size={40} color="white" style={{opacity: 0.9}} /><Text style={{color: 'white', fontSize: 20, fontWeight: 'bold', marginTop: 10}}>Cảm Hứng Hôm Nay</Text><Text style={{color: 'rgba(255,255,255,0.8)', fontSize: 12}}>Khơi nguồn sáng tạo cùng AI</Text></View>
                    <View style={dynamicStyles.musicContent}>
                        {musicMode === 'welcome' && (
                            <View style={{alignItems: 'center', width: '100%'}}><Ionicons name="bulb" size={50} color={colors.primary} style={{marginBottom: 10}} /><Text style={{color: colors.text, fontWeight: '600', fontSize: 16}}>Bạn đang bí ý tưởng?</Text><Text style={{color: colors.subText, fontSize: 13, marginBottom: 20, textAlign: 'center'}}>Chọn ngẫu nhiên hoặc nhập ý tưởng riêng.</Text><View style={{flexDirection: 'row', gap: 10, width: '100%'}}><TouchableOpacity onPress={handleRandomTopic} style={{flex: 1, backgroundColor: colors.iconBg, padding: 12, borderRadius: 8, alignItems: 'center', flexDirection: 'row', justifyContent: 'center'}}><Ionicons name="shuffle" size={16} color={colors.text} style={{marginRight: 5}} /><Text style={{color: colors.text, fontWeight: 'bold'}}>Ngẫu nhiên</Text></TouchableOpacity><TouchableOpacity onPress={() => setMusicMode('input')} style={{flex: 1, backgroundColor: colors.primary, padding: 12, borderRadius: 8, alignItems: 'center', flexDirection: 'row', justifyContent: 'center'}}><Ionicons name="create" size={16} color="white" style={{marginRight: 5}} /><Text style={{color: 'white', fontWeight: 'bold'}}>Tự nhập</Text></TouchableOpacity></View></View>
                        )}
                        {musicMode === 'input' && (
                            <View style={{width: '100%'}}><Text style={{fontWeight: 'bold', color: colors.text, marginBottom: 5}}>Ý tưởng sơ khai:</Text><TextInput style={[dynamicStyles.input, {height: 80, textAlignVertical: 'top'}]} placeholder="Ví dụ: Mèo phi hành gia nhớ nhà..." placeholderTextColor={colors.subText} multiline value={musicInput} onChangeText={setMusicInput}/><TouchableOpacity onPress={handleCustomMusicTopic} disabled={isGenerating} style={{backgroundColor: colors.primary, padding: 12, borderRadius: 8, alignItems: 'center', marginTop: 10}}>{isGenerating ? <ActivityIndicator color="white"/> : <Text style={{color: 'white', fontWeight: 'bold'}}>✨ Tạo Chủ Đề</Text>}</TouchableOpacity><TouchableOpacity onPress={() => setMusicMode('welcome')} style={{marginTop: 10, alignItems: 'center'}}><Text style={{color: colors.subText}}>Hủy</Text></TouchableOpacity></View>
                        )}
                        {musicMode === 'topic' && currentTopic && (
                            <View style={{width: '100%'}}><View style={{flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 15, borderBottomWidth: 1, borderBottomColor: colors.border, paddingBottom: 10}}><Text style={dynamicStyles.musicTitle}>{currentTopic.title}</Text><View style={{width: 40, height: 40, backgroundColor: colors.iconBg, borderRadius: 20, alignItems: 'center', justifyContent: 'center'}}><Ionicons name={(currentTopic.icon || 'musical-notes') as any} size={20} color={colors.primary} /></View></View><View style={{marginBottom: 10}}><Text style={{fontSize: 10, fontWeight: 'bold', color: colors.subText, textTransform: 'uppercase'}}>Thể loại</Text><Text style={{fontSize: 16, color: colors.primary, fontWeight: 'bold'}}>{currentTopic.genre}</Text></View><View style={{backgroundColor: colors.iconBg, padding: 12, borderRadius: 8, marginBottom: 10}}><Text style={{fontSize: 10, fontWeight: 'bold', color: colors.subText, textTransform: 'uppercase'}}>Ý tưởng chủ đạo</Text><Text style={{fontSize: 13, color: colors.text, fontStyle: 'italic', marginTop: 2}}>"{currentTopic.prompt}"</Text></View><View><Text style={{fontSize: 10, fontWeight: 'bold', color: colors.subText, textTransform: 'uppercase', marginBottom: 5}}>Vòng hợp âm gợi ý</Text><View style={{flexDirection: 'row', flexWrap: 'wrap'}}>{currentTopic.chord.split('-').map((c, i) => (<Text key={i} style={dynamicStyles.chordBadge}>{c.trim()}</Text>))}</View></View><TouchableOpacity onPress={composeFullSong} disabled={isGenerating} style={{backgroundColor: 'transparent', borderWidth: 1, borderColor: colors.primary, padding: 12, borderRadius: 25, alignItems: 'center', marginTop: 20, flexDirection: 'row', justifyContent: 'center'}}>{isGenerating ? <ActivityIndicator color={colors.primary}/> : (<><Ionicons name="sparkles" size={16} color={colors.primary} style={{marginRight: 5}} /><Text style={{color: colors.primary, fontWeight: 'bold'}}>Viết Nhạc & Lời (AI)</Text></>)}</TouchableOpacity><TouchableOpacity onPress={() => setMusicMode('welcome')} style={{marginTop: 10, alignItems: 'center'}}><Text style={{color: colors.subText, fontSize: 12}}>Chọn chủ đề khác</Text></TouchableOpacity></View>
                        )}
                    </View>
                </View>
                {songResult && (
                    <View style={{backgroundColor: colors.card, padding: 15, borderRadius: 10, borderWidth: 1, borderColor: colors.border, marginBottom: 20}}>
                         <View style={{flexDirection: 'row', alignItems: 'center', marginBottom: 10}}><View style={{backgroundColor: colors.iconBg, padding: 5, borderRadius: 5, marginRight: 8}}><Ionicons name="pencil" size={16} color={colors.primary} /></View><Text style={{fontWeight: 'bold', color: colors.text, textTransform: 'uppercase'}}>Bản Thảo Hoàn Chỉnh</Text></View>
                         <View style={{backgroundColor: colors.iconBg, padding: 10, borderRadius: 8, marginBottom: 10, borderWidth: 1, borderColor: colors.border}}><Text style={{fontWeight: 'bold', fontSize: 16, color: colors.primary, marginBottom: 5}}>{songResult.song_title}</Text><View style={{flexDirection: 'row', flexWrap: 'wrap', gap: 5}}><Text style={{fontSize: 11, color: colors.subText}}>🎵 {songResult.song_genre}</Text><Text style={{fontSize: 11, color: colors.subText}}>🥁 {songResult.song_tempo}</Text><Text style={{fontSize: 11, color: colors.subText}}>🎤 {songResult.song_vocals}</Text></View></View>
                         <View style={{backgroundColor: colors.iconBg, padding: 15, borderRadius: 8, borderWidth: 1, borderColor: colors.border}}><View style={{flexDirection: 'row', justifyContent: 'space-between', marginBottom: 5}}><Text style={{fontSize: 10, fontWeight: 'bold', color: colors.subText, textTransform: 'uppercase'}}>Lời & Hợp âm</Text><TouchableOpacity onPress={() => copyToClipboard(songResult.song_lyrics.replace(/<br>/g, '\n'))}><Text style={{fontSize: 10, color: colors.primary, fontWeight: 'bold'}}>Copy</Text></TouchableOpacity></View><Text style={{fontSize: 13, color: colors.text, lineHeight: 22}}>{songResult.song_lyrics.split('<br>').map((line, i) => (<Text key={i}>{line}{'\n'}</Text>))}</Text></View>
                         {songResult.song_abc_notation && (<View style={{marginTop: 10, padding: 10, backgroundColor: colors.iconBg, borderRadius: 8, borderWidth: 1, borderColor: colors.border}}><Text style={{fontSize: 10, fontWeight: 'bold', color: colors.success, marginBottom: 5}}>SHEET NHẠC (ABC NOTATION)</Text><Text style={{fontFamily: 'monospace', fontSize: 10, color: colors.text}}>{songResult.song_abc_notation}</Text><TouchableOpacity onPress={() => copyToClipboard(songResult.song_abc_notation)} style={{marginTop: 5}}><Text style={{fontSize: 10, color: colors.primary, fontWeight: 'bold'}}>Copy mã này dán vào abcjs.net</Text></TouchableOpacity></View>)}
                         <View style={{marginTop: 20, paddingTop: 15, borderTopWidth: 1, borderTopColor: colors.border, borderStyle: 'dashed'}}><Text style={{textAlign: 'center', fontSize: 12, fontWeight: 'bold', color: colors.subText, marginBottom: 10, textTransform: 'uppercase'}}>✨ Studio Sáng Tạo AI</Text><View style={dynamicStyles.studioGrid}><TouchableOpacity onPress={() => generateExtendedContent('arrange')} style={dynamicStyles.studioBtn}><Ionicons name="options" size={16} color="#EC4899" /><Text style={dynamicStyles.studioBtnText}>Hòa Âm</Text></TouchableOpacity><TouchableOpacity onPress={() => generateExtendedContent('translate')} style={dynamicStyles.studioBtn}><Ionicons name="language" size={16} color="#3B82F6" /><Text style={dynamicStyles.studioBtnText}>Dịch Anh</Text></TouchableOpacity><TouchableOpacity onPress={() => generateExtendedContent('art')} style={dynamicStyles.studioBtn}><Ionicons name="image" size={16} color="#10B981" /><Text style={dynamicStyles.studioBtnText}>Ảnh Bìa</Text></TouchableOpacity><TouchableOpacity onPress={() => generateExtendedContent('critic')} style={dynamicStyles.studioBtn}><Ionicons name="star" size={16} color="#F59E0B" /><Text style={dynamicStyles.studioBtnText}>Phê Bình</Text></TouchableOpacity><TouchableOpacity onPress={() => generateExtendedContent('mv')} style={dynamicStyles.studioBtn}><Ionicons name="videocam" size={16} color="#EF4444" /><Text style={dynamicStyles.studioBtnText}>Kịch Bản</Text></TouchableOpacity><TouchableOpacity onPress={() => generateExtendedContent('theory')} style={dynamicStyles.studioBtn}><Ionicons name="school" size={16} color="#8B5CF6" /><Text style={dynamicStyles.studioBtnText}>Nhạc Lý</Text></TouchableOpacity></View>{isExtendedLoading && <ActivityIndicator color={colors.primary} style={{marginTop: 10}} />}{extendedContent && (<View style={{marginTop: 10, padding: 15, backgroundColor: colors.card, borderRadius: 8, borderWidth: 1, borderColor: colors.border, borderLeftWidth: 4, borderLeftColor: colors.primary}}><View style={{flexDirection:'row', justifyContent:'space-between', marginBottom: 5}}><Text style={{fontWeight: 'bold', color: colors.primary}}>{extendedTitle}</Text><TouchableOpacity onPress={() => setExtendedContent(null)}><Ionicons name="close" size={16} color={colors.subText}/></TouchableOpacity></View><Text style={{color: colors.text, fontSize: 13, lineHeight: 20}}>{extendedContent}</Text><TouchableOpacity onPress={() => copyToClipboard(extendedContent)} style={{marginTop: 10}}><Text style={{fontSize: 11, fontWeight:'bold', color: colors.primary}}>Copy nội dung</Text></TouchableOpacity></View>)}</View>
                    </View>
                )}
            </View>
          )}
          ...
          {mediaType === 'video' && (
             <View>
                 <View style={{flexDirection: 'row', justifyContent: 'space-between', marginBottom: 10}}>
                    <View style={{flex: 0.48}}><Text style={dynamicStyles.label}>Max ký tự NV:</Text><TextInput style={dynamicStyles.input} keyboardType="numeric" value={limitCharDesc} onChangeText={setLimitCharDesc} placeholder="200" placeholderTextColor={colors.subText} /></View>
                    <View style={{flex: 0.48}}><Text style={dynamicStyles.label}>Max ký tự Video:</Text><TextInput style={dynamicStyles.input} keyboardType="numeric" value={limitMainPrompt} onChangeText={setLimitMainPrompt} placeholder="1000" placeholderTextColor={colors.subText} /></View>
                </View>
                    ...
                    <View style={{marginTop: 25}}>
                        {detectedChars.length > 0 && (
                            <View style={{marginBottom: 15}}>
                                <Text style={[dynamicStyles.label, {color: colors.primary}]}>👥 NHÂN VẬT:</Text>
